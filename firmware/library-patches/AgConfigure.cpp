...
JSON_PROP_DEF(ptUploadSec);
JSON_PROP_DEF(ptPmReadSec);
JSON_PROP_DEF(ptSensorMode);
JSON_PROP_DEF(ptWarmupSec);
...
#define jprop_ptUploadSec_default                     600
#define jprop_ptPmReadSec_default                     300
#define jprop_ptSensorMode_default                    "duty"
#define jprop_ptWarmupSec_default                     60

static const int PLANTOWER_UPLOAD_SEC_MIN = 60;
static const int PLANTOWER_UPLOAD_SEC_MAX = 3600;
static const int PLANTOWER_PM_READ_SEC_MIN = 2;
static const int PLANTOWER_PM_READ_SEC_MAX = 3600;
static const int PLANTOWER_WARMUP_SEC_MIN = 10;
static const int PLANTOWER_WARMUP_SEC_MAX = 180;

JSONVar jconfig;

static int clampPlantowerInt(int value, int minValue, int maxValue) {
  if (value < minValue) {
    return minValue;
  }
  if (value > maxValue) {
...
static String normalizePlantowerSensorMode(String mode) {
  mode.trim();
  mode.toLowerCase();
  if (mode == "duty" || mode == "duty-cycle" || mode == "sleep") {
    return String("duty");
  }
...
bool Configuration::updatePmCorrection(JSONVar &json) {
  ...
  bool useEPA = (bool)slr["useEpa2021"];
...
void Configuration::saveConfig(void) {
  ...
  int len = data.length();
#ifdef ESP8266
  if (len >= EEPROM_CONFIG_SIZE) {
    logError("Config too large for EEPROM");
    return;
  }
  for (int i = 0; i < EEPROM_CONFIG_SIZE; i++) {
    EEPROM.write(i, i < len ? data[i] : 0);
  }
  EEPROM.commit();
#else
...
void Configuration::defaultConfig(void) {
  ...
  jconfig[jprop_ptUploadSec] = jprop_ptUploadSec_default;
  jconfig[jprop_ptPmReadSec] = jprop_ptPmReadSec_default;
  jconfig[jprop_ptSensorMode] = jprop_ptSensorMode_default;
  jconfig[jprop_ptWarmupSec] = jprop_ptWarmupSec_default;
...
Configuration::Configuration(Stream &debugLog)
    : PrintLog(debugLog, "Configure"), _callback(NULL) {}
...
void Configuration::toConfig(const char *buf) {
  ...
  if (JSON.typeof_(jconfig[jprop_configurationControl]) != "string") {
    isConfigFieldInvalid = true;
  } else {
    String ctrl = jconfig[jprop_configurationControl];
    if (ctrl != String(CONFIGURATION_CONTROL_NAME
  ...
  /** Validate Plantower local upload interval */
  if (JSON.typeof_(jconfig[jprop_ptUploadSec]) != "number") {
    isConfigFieldInvalid = true;
  } else {
    int value = jconfig[jprop_ptUploadSec];
    if (value < PLANTOWER_UPLOAD_SEC_MIN || value > PLANTOWER_UPLOAD_SEC_MAX) {
      isConfigFieldInvalid = true;
    } else {
      isConfigFieldInvalid = false;
    }
  ...
  if (isConfigFieldInvalid) {
    jconfig[jprop_ptUploadSec] = jprop_ptUploadSec_default;
    changed = true;
    logInfo("toConfig: ptUploadSec changed");
  }

  /** Validate Plantower local PM read interval */
  if (JSON.typeof_(jconfig[jprop_ptPmReadSec]) != "number") {
    isConfigFieldInvalid = true;
  } else {
    int value = jconfig[jprop_ptPmReadSec];
    if (value < PLANTOWER_PM_READ_SEC_MIN ||
        value > PLANTOWER_PM_READ_SEC_MAX) {
      isConfigFieldInvalid = true;
    } else {
      isConfigFieldInvalid = false;
    }
  ...
  if (isConfigFieldInvalid) {
    jconfig[jprop_ptPmReadSec] = jprop_ptPmReadSec_default;
    changed = true;
    logInfo("toConfig: ptPmReadSec changed");
  }

  /** Validate Plantower local sensor mode */
  if (JSON.typeof_(jconfig[jprop_ptSensorMode]) != "string") {
    isConfigFieldInvalid = true;
  } else {
    String value = normalizePlantowerSensorMode(jconfig[jprop_ptSensorMode]);
    if (value != String(jconfig[jprop_ptSensorMode])) {
      isConfigFieldInvalid = true;
    } else {
      isConfigFieldInvalid = false;
    }
  ...
  if (isConfigFieldInvalid) {
    jconfig[jprop_ptSensorMode] = jprop_ptSensorMode_default;
    changed = true;
    logInfo("toConfig: ptSensorMode changed");
  }

  /** Validate Plantower local warmup time */
  if (JSON.typeof_(jconfig[jprop_ptWarmupSec]) != "number") {
    isConfigFieldInvalid = true;
  } else {
    int value = jconfig[jprop_ptWarmupSec];
    if (value < PLANTOWER_WARMUP_SEC_MIN ||
        value > PLANTOWER_WARMUP_SEC_MAX) {
      isConfigFieldInvalid = true;
    } else {
      isConfigFieldInvalid = false;
    }
  ...
  if (isConfigFieldInvalid) {
    jconfig[jprop_ptWarmupSec] = jprop_ptWarmupSec_default;
    changed = true;
    logInfo("toConfig: ptWarmupSec changed");
  }
...
int Configuration::getPlantowerUploadIntervalSec(void) {
  int value = jconfig[jprop_ptUploadSec];
  return clampPlantowerInt(value, PLANTOWER_UPLOAD_SEC_MIN,
                           PLANTOWER_UPLOAD_SEC_MAX);
}

int Configuration::getPlantowerPmReadIntervalSec(void) {
  int value = jconfig[jprop_ptPmReadSec];
  return clampPlantowerInt(value, PLANTOWER_PM_READ_SEC_MIN,
                           PLANTOWER_PM_READ_SEC_MAX);
}

int Configuration::getPlantowerWarmupSec(void) {
  int value = jconfig[jprop_ptWarmupSec];
  return clampPlantowerInt(value, PLANTOWER_WARMUP_SEC_MIN,
                           PLANTOWER_WARMUP_SEC_MAX);
}

int Configuration::getPlantowerUploadIntervalMs(void) {
  return getPlantowerUploadIntervalSec() * 1000;
}

int Configuration::getPlantowerPmReadIntervalMs(void) {
  return getPlantowerPmReadIntervalSec() * 1000;
}

int Configuration::getPlantowerWarmupMs(void) {
  return getPlantowerWarmupSec() * 1000;
}

String Configuration::getPlantowerSensorMode(void) {
  if (JSON.typeof_(jconfig[jprop_ptSensorMode]) != "string") {
    return String(jprop_ptSensorMode_default);
  }
  return normalizePlantowerSensorMode(String(jconfig[jprop_ptSensorMode]));
}

bool Configuration::isPlantowerDutyCycleEnabled(void) {
  return getPlantowerSensorMode() == String("duty");
}

bool Configuration::setPlantowerLocalSettings(int uploadSec, int pmReadSec,
                                              String sensorMode,
                                              int warmupSec) {
  uploadSec = clampPlantowerInt(uploadSec, PLANTOWER_UPLOAD_SEC_MIN,
                                PLANTOWER_UPLOAD_SEC_MAX);
  pmReadSec = clampPlantowerInt(pmReadSec, PLANTOWER_PM_READ_SEC_MIN,
                                PLANTOWER_PM_READ_SEC_MAX);
  warmupSec = clampPlantowerInt(warmupSec, PLANTOWER_WARMUP_SEC_MIN,
                                PLANTOWER_WARMUP_SEC_MAX);
  sensorMode = normalizePlantowerSensorMode(sensorMode);

  bool changed = false;
  if (getPlantowerUploadIntervalSec() != uploadSec) {
    jconfig[jprop_ptUploadSec] = uploadSec;
    changed = true;
  }
  if (getPlantowerPmReadIntervalSec() != pmReadSec) {
    jconfig[jprop_ptPmReadSec] = pmReadSec;
    changed = true;
  }
  if (getPlantowerSensorMode() != sensorMode) {
    jconfig[jprop_ptSensorMode] = sensorMode;
    changed = true;
  }
  if (getPlantowerWarmupSec() != warmupSec) {
    jconfig[jprop_ptWarmupSec] = warmupSec;
    changed = true;
  }
  ...
  if (changed) {
    updated = true;
    saveConfig();
    logInfo("Plantower local settings saved");
  }
  return changed;
}
